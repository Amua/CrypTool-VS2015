...
namespace act
{
	...
	public:
		...
		virtual void GenerateRSAKeyPair(ISCardOS* os, const OSKeyInfo& ki) = 0;
		virtual void ImportRSAKeyPair(ISCardOS* os, const OSKeyInfo& ki, const Blob& p, const Blob& q, const Blob& priv_exp, Blob& pubkey) = 0;
		virtual void DeletePrivateKeyObj(ISCardOS* os, unsigned short fid) = 0;

		virtual void doVerifyPinCmd(const ITokenPIN* pin, const Blob& value) = 0;
		virtual void doChangePinCmd(const ITokenPIN* token_pin, const Blob& oldnew) = 0;
		virtual void doSetPinValueCmd(const ITokenPIN* token_pin, const Blob& newval, const Location<>&) = 0;
		virtual void InitPin(const ITokenPIN* token_pin, const Blob& new_pin, const Location<>&) = 0;

		virtual void SetKeySlots(ISCardOS* os) = 0;
	};
