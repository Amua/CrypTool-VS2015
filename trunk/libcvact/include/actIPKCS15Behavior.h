...
namespace act
{
	...
	public:
		...
		virtual TokenType	GetTokenType(const IToken* token) const = 0;
		virtual void		InitProfileCapabilities(ITokenConfig* tkcfg) const = 0;
		...
		virtual FIDType		GetFirstFID(byte obj_type, const PKCS15Object& p15object) const = 0;
		virtual void		GetObjectInfo(byte obj_type, PKCS15Object& p15object, bool needs_object_ref) const = 0;
		
		virtual FIDType		TransformKeyReference(FIDType id, bool to_on_card_reference = false, bool* bReference = 0) const = 0;

		virtual void		GetRelativeFile(Blob& absolute_filepath) const = 0;

		virtual const Blob&	GetACTokenRights() const = 0;
		virtual Blob		GetAccessRights(const ITokenPIN* pin, byte obj_type, ulong usage) const = 0;
		virtual Blob		GetAccessRights(IAuthIdRef* authIdRef, IAuthIdRef* parentIdRef, byte obj_type, ulong usage) const = 0;
