...
namespace act
{
	...
	//
	// Get OID from ASN.1 encoded parameters (if unknown, an empty Blob is returned).
	Blob GetCurveOID(const Blob& curve_param);

	// CV certificate (ISO 7816-8) public key decoding
	bool hasISOPuKTag(const Blob& pk_info);
	bool ISOPuKToECPoint(const Blob& pk_info, Blob& public_point);
	void ISOPuKToECKey(const Blob& pk_info, IKey* ec_key);
	void ECKeyToISO(const IKey* ec_key, const Blob& oid, Blob& key_info, int output_type = CURVEPARAM);
