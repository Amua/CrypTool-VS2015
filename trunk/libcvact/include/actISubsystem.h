...
namespace act
{
	...
	enum subsystem_paramid_t
	{
		...
		SUBSYSTEM_READ_FEATURES,				//!< Set to \a PARAM_FALSE to disable reading of reader-features during initialization.
		SUBSYSTEM_SHARE_CONTEXT,				//!< Set to \a PARAM_TRUE to share context with \a PCSCMonitor.
		...
		SUBSYSTEM_SHUTDOWN_NO_CONTEXT_RELEASE,	//!< If \a PARAM_TRUE do not release context (f.e. SCardReleaseContext) if already shutdown.
		SUBSYSTEM_ENABLE_INTERACTIVITY,			//!< If \a PARAM_TRUE subsystem is allowed to show GUI, like PIN dialog etc.
		// RFU									: 8 - 12
		SUBSYSTEM_PARAM_CUSTOM_BEGIN			= 13
		// Custom								: 13 - 15
	};
