...
namespace act
{
	...
	enum tc_paramid_t
	{
		...
		TC_FILECACHE_PATH,
		/* ... */
		TC_END,								//!< One past last tc_paramid_t, \see TC_SERIALIZE_BEGIN.
	...
	ACT_DEFINE_ENUM_FLAG_OPERATORS(tc_paramid_t);

	enum tc_serializeid_t
	{
		TC_SERIALIZE_BEGIN					/*!< Begin of the card serialized flags.		 */ = TC_END,
		TC_ENABLE_NXP_SM_ACCELERATOR_PATCH	/*!< Bit 0 - Set if card has SM Accelerator Bug. */ = TC_SERIALIZE_BEGIN,
		TC_IS_ISO_MOC,						//!< Bit 1 - If /p TC_IS_MOC_ENABLED is ISO compliant.
		TC_ENABLE_IFX_SM_ACCELERATOR_PATCH,	//!< Bit 2 - Set if card has SM Accelerator Bug.
		TC_DISABLE_EXTENDED_APDU,			//!< Bit 3 - If True, OS extended APDU flag will be false.
		/* ... */
		TC_SERIALIZE_END,					//!< One past last card serialized flag.
		TC_SERIALIZE_LENGTH					//!< Maximum number of encoded bits.
											= TC_SERIALIZE_END - TC_SERIALIZE_BEGIN,
	};
